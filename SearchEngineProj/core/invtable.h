...
public:
	...
	bool load(Creader&reader,int doc_id = -1){//需以输入的数据建表 并指定所在文档（默认以自动计数作为文档号）
		if (doc_id < 0)
			doc_id = doc_count;
		string buf;
		int bias = 0;
	...
	record_table*search_string(string s,int k = 1){//k近邻查找
		for (int i = 0; i < mem_tb.size(); i++){
			if (mem_tb[i].token == s)return &mem_tb[i];
		}
